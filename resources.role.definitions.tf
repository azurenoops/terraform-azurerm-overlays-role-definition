...
resource "azurerm_role_definition" "custom_role" {
  ...
  scope       = lookup(var.custom_role_definitions[count.index], "scope", data.azurerm_client_config.current.subscription_id)
  ...
  assignable_scopes = lookup(var.custom_role_definitions[count.index], "assignable_scopes", [data.azurerm_client_config.current.subscription_id])
}
